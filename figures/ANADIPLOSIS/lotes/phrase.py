...
if os.path.basename(directorio_actual) == "lotes":
    ...
    path_resultados = os.path.join(directorio_actual, "ANA")
...
for archivo_palabras in archivos_lotes:
    ...
    patron_busqueda = os.path.join(
        path_resultados, f"anadiplosis_lote_{lote_id}*.txt"
    )
    ...
    for i in range(1, len(bloques), 2):
        ...
        for poema in versiones_poema:
            ...
            entry = {
                "text": f"Escribe un poema usando la figura retórica \"anadiplosis\" con la palabra \"{palabra_clave}\".\nPoema:\n{poema_final}\n{tokenizer_eos}"
            }
...
output_file = "dataset_final_anadiplosis.json"
